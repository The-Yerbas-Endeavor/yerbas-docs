...
```bash
wget https://github.com/Raptor3um/Yerbas/releases/download/1.0.15.1/yerbas-1.0.15.1-x86_64-linux-gnu.tar.xz
tar -xvf yerbas-1.0.15.1-x86_64-linux-gnu.tar.xz
mv yerbas-1.0.15.1-x86_64-linux-gnu raptoreum_live
```
...
```bash
mkdir ~/.raptoreumcore
nano ~/.raptoreumcore/yerbas.conf
```
...
Edit pool_configs/yerbas.json as needed, make sure user and password matches what you have in <code>~/.raptoreumcore/yerbas.conf</code>.
