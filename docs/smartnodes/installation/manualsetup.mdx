...
mkdir -p rtm_latest
cd rtm_latest
wget https://github.com/The-Yerbas-Endeavor/yerbas/releases/download/1.3.17.01/yerbas-ubuntu20-1.3.17.01.tar.gz
...
cp -R blocks chainstate evodb llmq ~/.yerbascore
echo "daemon=1" >> ~/.yerbascore/yerbas.conf
~/rtm_latest/yerbas-ubuntu20-1.3.17.01/./yerbasd
echo all done!
```
