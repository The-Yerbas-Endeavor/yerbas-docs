...
mkdir -p rtm_latest
cd rtm_latest
wget https://github.com/Raptor3um/yerbas/releases/download/1.3.17.01/yerbas-ubuntu20-1.3.17.01.tar.gz
tar -xvf yerbas-ubuntu20-1.3.17.01.tar.gz
cd ..
wget https://bootstrap.yerbas.com/bootstraps_for_v1.3.17.00/bootstrap.zip
unzip bootstrap.zip
mkdir ~/.raptoreumcore && touch ~/.raptoreumcore/yerbas.conf
cp -R blocks chainstate evodb llmq ~/.raptoreumcore
echo "daemon=1" >> ~/.raptoreumcore/yerbas.conf
~/rtm_latest/yerbas-ubuntu20-1.3.17.01/./raptoreumd
echo all done!
```
...
- Send 1 million YERB to self (this is current collateral amount)
...
- <code>Fee address</code>: This is any address in your wallet which contains
  enough YERB to pay the fee (cannot be the address which you sent the 1 million
